...
public interface TransactionRepository extends JpaRepository<Transaction, Long>,
    ...
    @Query(value = "SELECT COUNT(*) FROM transaction t LEFT JOIN account a ON t.from_account_id = a.id WHERE a.id IS NULL", nativeQuery = true)
    long countOrphanFromAccounts();

    @Query(value = "SELECT COUNT(*) FROM transaction t LEFT JOIN account a ON t.to_account_id = a.id WHERE a.id IS NULL", nativeQuery = true)
